    ...
    <div class="flex flex-col gap-2 w-full items-center pb-20">
        ...
        <div class="flex items-center justify-center w-full mb-4">
            <button data-tooltip-target="tooltip-sell"
                ...
                <v-icon name="md-shoppingcart-round" scale="1" class="w-6 h-6 mb-1 text-body group-hover:text-fg-brand" />
                <span class="sr-only">{{ t('shop.sell') }}</span>
            </button>
            <div id="tooltip-sell" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-dark rounded-base shadow-xs opacity-0 tooltip">
                {{ t('shop.sell_item') }}
            ...
            <div id="tooltip-sold" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-dark rounded-base shadow-xs opacity-0 tooltip">
                {{ t('shop.sold_history') }}
                <div class="tooltip-arrow" data-popper-arrow></div>
            </div>
        </div>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
                    <div class="flex items-center justify-between border-b border-default pb-4 md:pb-5">
                        ...
                        <button type="button" class="text-body bg-transparent hover:bg-neutral-tertiary hover:text-heading rounded-base text-sm w-9 h-9 ms-auto inline-flex justify-center items-center" data-modal-hide="sell-modal">
                    ...
                    <form @submit.prevent="handleSellSubmit">
                        <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                            ...
                            <div class="col-span-2 bg-neutral-secondary-medium border border-default p-3 rounded-base text-xs space-y-1">
                                <div class="flex justify-between">
                                    ...
                                    <span :class="targetInventoryItem && targetInventoryItem.quantity > 0 ? 'text-heading font-bold' : 'text-danger font-bold'">
                                        {{ targetInventoryItem ? `${targetInventoryItem.quantity} ${t('common.units')}` : t('shop.out_of_stock') }}
                                    </span>
                                </div>
                                <div class="flex justify-between" v-if="targetInventoryItem">
                                    <span class="text-body font-medium">{{ t('shop.selling_price_label') }}</span>
                                    <span class="text-heading font-bold">{{ targetInventoryItem.sellingPrice }} {{ t('shop.selling_price_unit') }}</span>
                                </div>
                            </div>
                            <div class="col-span-2">
                                <label for="sell-quantity" class="block mb-2.5 text-sm font-medium text-heading">{{ t('shop.quantity_to_sell') }}</label>
                            ...
                            <div class="col-span-2" v-if="targetInventoryItem && sellQuantity">
                                <div class="flex justify-between items-center p-3 bg-brand-softer border border-brand-subtle rounded-base text-sm font-medium text-fg-brand-strong">
                                    ...
                                    <span class="text-base font-bold">{{ (sellQuantity || 0) * (targetInventoryItem.sellingPrice || 0) }} {{ t('common.br') }}</span>
                                </div>
                            </div>
                            <div class="col-span-2" v-if="sellErrorMessage">
                                <div class="p-3 bg-danger-softer border border-danger-subtle rounded-base text-xs font-semibold text-danger-strong">⚠️ {{ sellErrorMessage }}</div>
                            </div>
                            <div class="col-span-2" v-if="sellSuccessMessage">
                                <div class="p-3 bg-success-softer border border-success-subtle rounded-base text-xs font-semibold text-fg-success">✓ {{ sellSuccessMessage }}</div>
                            </div>
                        </div>
                        <div class="flex items-center space-x-4 border-t border-default pt-4 md:pt-6">
                            ...
                            <button data-modal-hide="sell-modal" type="button" class="text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading focus:ring-4 focus:ring-neutral-tertiary shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">
                                {{ t('common.close') }}
                            </button>
                        </div>
                    </form>
