...
const filteredInventory = computed(() => {
    ...
    return inventory.value.filter(inv => inv.item.name.toLowerCase().includes(q));
});
...
const handleAddStock = async () => {
    ...
    if (!selectedItemForAdd.value) {
        addStockErrorMessage.value = t('inventory.err_select_item');
        return;
    }
...
<template>
    <div class="flex flex-col gap-2 w-full items-center pb-20">
        ...
        <div class="flex items-center justify-center w-full mb-4">
            <!-- Restock History -->
            <router-link
                :to="selectedInventoryItem ? `/inventory/history/${selectedInventoryItem.id}` : '/inventory/history'"
            ...
            <router-link to="/inventory/low-stock" data-tooltip-target="tooltip-low-stock"
                ...
                <span v-if="lowStockCount > 0"
                    class="absolute -top-1 right-3 bg-danger text-white text-2xs font-bold px-1.5 py-0.5 rounded-full">
                    {{ lowStockCount }}
                </span>
            ...
            <!-- Delete -->
            <button data-tooltip-target="tooltip-delete" data-modal-target="delete-modal"
                data-modal-toggle="delete-modal" @click.capture="handleDeleteClick"
                    ...
                    <form @submit.prevent="handleAddStock">
                        <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                            <div class="col-span-2">
                                <label for="item-select" class="block mb-2.5 text-sm font-medium text-heading">{{
                                    t('inventory.item_label') }}</label>
                            ...
                            <div class="col-span-2">
                                <label for="quantity" class="block mb-2.5 text-sm font-medium text-heading">{{
                                    t('inventory.quantity_label') }}</label>
                            ...
                            <div class="col-span-2 sm:col-span-1">
                                <label for="acquired-price" class="block mb-2.5 text-sm font-medium text-heading">{{
                                    t('inventory.acquired_price_label') }}</label>
                                <input type="number" step="any" min="0" id="acquired-price"
                                    v-model="acquiredPriceForAdd"
                                    class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                    :placeholder="t('inventory.acquired_price_placeholder')" required>
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label for="selling-price" class="block mb-2.5 text-sm font-medium text-heading">{{
                                    t('inventory.selling_price_label') }}</label>
                        ...
                        <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                            <div class="col-span-2">
                                <label class="block mb-2.5 text-sm font-medium text-heading">{{
                                    t('inventory.item_label') }}</label>
                                <select disabled
                                    class="block w-full px-3 py-2.5 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base shadow-xs disabled:opacity-60 disabled:cursor-not-allowed">
                                    <option selected>{{ restockItem?.item.name }}</option>
                                </select>
                            </div>
                            
                            <div class="col-span-2">
                                <label for="restock-quantity" class="block mb-2.5 text-sm font-medium text-heading">{{
                                    t('inventory.restock_quantity_label') }}</label>
                            ...
                            <div class="col-span-2 sm:col-span-1">
                                <label for="restock-acquired-price"
                                    class="block mb-2.5 text-sm font-medium text-heading">{{
                                        t('inventory.acquired_price_label') }}</label>
                                <input type="number" step="any" min="0" id="restock-acquired-price"
                                    v-model="restockAcquiredPrice"
                                    class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                    :placeholder="t('inventory.acquired_price_placeholder')" required>
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label for="restock-selling-price"
                                    class="block mb-2.5 text-sm font-medium text-heading">{{
                                        t('inventory.selling_price_label') }}</label>
                                <input type="number" step="any" min="0" id="restock-selling-price"
                                    v-model="restockSellingPrice"
                                    class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                    :placeholder="t('inventory.selling_price_placeholder')" required>
                            </div>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-body">
                <thead class="text-sm text-body bg-neutral-secondary-medium border-b border-default-medium">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 font-medium">{{ t('inventory.col_unit') }}</th>
                        ...
                        <th scope="col" class="px-6 py-3 font-medium text-right"><span class="sr-only">{{
                            t('inventory.restock_btn') }}</span></th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="inventoryItem in filteredInventory" :key="inventoryItem.id" :class="{
                        ...
                        <td class="px-6 py-4 font-semibold"
                            :class="{ 'text-danger-strong font-bold': inventoryItem.quantity <= inventoryItem.runOutThreshhold }">
                            {{ inventoryItem.quantity }}
                            <span v-if="inventoryItem.quantity <= inventoryItem.runOutThreshhold"
                                class="ms-2 bg-danger-softer text-danger-strong border border-danger-subtle text-xs px-2 py-0.5 rounded-full font-bold">
                        ...
                        <td class="px-6 py-4 text-right" @click.stop>
                            <button data-modal-target="restock-inventory-modal"
                                data-modal-toggle="restock-inventory-modal" @click="openRestockModal(inventoryItem)"
                                class="font-medium text-fg-brand hover:underline">{{ t('inventory.restock_btn')
                                }}</button>
                        </td>
                    </tr>
