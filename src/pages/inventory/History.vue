...
watch(inventoryId, async () => { await loadHistory(); });
                ...
                <li v-for="log in filteredHistory" :key="log.id" class="mb-10 ms-6">
                    ...
                    <div class="mb-4 text-sm text-body space-y-1">
                        ...
                        <div class="flex items-center gap-4 text-xs font-medium">
                            ...
                            <span class="text-fg-success font-semibold">{{ t('history.margin') }} +{{ (log.sellingPrice || 0) - (log.acquiredPrice || 0) }} {{ t('common.etb') }}/{{ t('common.units') }}</span>
                        </div>
                    </div>
                </li>
            </ol>
            <div v-else class="text-center py-10 text-body">{{ t('history.no_records') }}</div>
        </div>
    </div>
</template>
