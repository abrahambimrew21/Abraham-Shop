...
const filteredVarients = computed(() => {
    if (!item.value || !item.value.varient) return [];
    if (!searchQuery.value.trim()) return item.value.varient;
    const q = searchQuery.value.toLowerCase().trim();
    return item.value.varient.filter(v =>
...
const toggleSelectVarient = (v: ItemVarient) => {
    if (selectedVarient.value?.id === v.id) {
        selectedVarient.value = null;
    } else {
        selectedVarient.value = v;
    }
};
...
const handleAddVarient = async () => {
    ...
    const updatedVarients = [...(item.value.varient || []), newVarient];
    item.value.varient = updatedVarients;
    ...
    <div class="flex flex-col gap-2 w-full items-center pb-20">
        <div class="flex items-center gap-2">
            <h2 class="text-xl font-medium text-primary-900">
                {{ item ? `${item.name} Varients` : 'Item Varients' }}
            </h2>
        </div>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-6 text-body">Are you sure you want to delete {{ selectedVarient?.name || 'this varient' }}?
                        </h3>
        ...
        <!-- Add Varient Modal -->
        <div id="add-varient-modal" tabindex="-1" aria-hidden="true"
            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
                    <div class="flex items-center justify-between border-b border-default pb-4 md:pb-5">
                        <h3 class="text-lg font-medium text-heading">
                            Create new varient for {{ item?.name }}
                        </h3>
                        <button type="button"
                            class="text-body bg-transparent hover:bg-neutral-tertiary hover:text-heading rounded-base text-sm w-9 h-9 ms-auto inline-flex justify-center items-center"
                            data-modal-hide="add-varient-modal">
                            <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                                height="24" fill="none" viewBox="0 0 24 24">
                    ...
                    <form @submit.prevent="handleAddVarient">
                        <div class="grid gap-4 grid-cols-2 py-4 md:py-6">
                            <div class="col-span-2 sm:col-span-1">
                                <label for="varient-name"
                                    class="block mb-2.5 text-sm font-medium text-heading">Varient Name</label>
                                <input type="text" name="varient-name" id="varient-name" v-model="newVarientName"
                                    class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                    placeholder="e.g. 500ml" required>
                            </div>
                            <div class="col-span-2 sm:col-span-1">
                                <label for="unit"
                                    class="block mb-2.5 text-sm font-medium text-heading">Unit</label>
                                <input type="text" name="unit" id="unit" v-model="newUnit"
                                    class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                                    placeholder="e.g. ml, kg, pcs" required>
                            </div>
                        </div>
                ...
                <tbody>
                    ...
                    <tr v-if="filteredVarients.length === 0">
                        <td colspan="4" class="px-6 py-8 text-center text-body">
                            No varients found for this item.
                        </td>
                    </tr>
                </tbody>
            </table>
